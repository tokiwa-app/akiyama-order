...
  // ★ここから追加: humanReadableId の生成
  let humanReadableId = null;
  if (faxAt) {
    // faxAt は UTC で保存されているため、UTCのメソッドを使用
...
      // 2. 案件情報
      customerName: "",        // 得意先名（初期値）
      deliveryDate: null,      // 納期（初期値）

      // 3. 工程管理ステータス (定尺, シャー, レーザー, 曲げ)
      processStatusSeichaku: "", // 定尺（初期値）
      processStatusShear: "",    // シャー（初期値）
      processStatusLaser: "",    // レーザー（初期値）
      processStatusBending: "",  // 曲げ（初期値）

      // 4. 全体ステータス
      deliveryStatus: "",      // 納品（初期値）
      stockStatus: "",         // 在庫（初期値）
    },
    { merge: true }
  );
